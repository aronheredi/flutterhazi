...
void main() {
  ...
  group(
    'LoginPage valid input sends correct event [2]',
    () {
      var emails = [
        'alpaca@gmail.com',
        'llama@gmail.com',
      ...
      for (var email in emails) {
        for (var password in passwords) {
          for (var rememberMe in rememberMeValues) {
            testWidgets(
                'LoginPage correct field test (email: $email, password: $password, rememberMe:$rememberMe)',
                (tester) async {
              var bloc = MockLoginBloc();
              whenListen(bloc, const Stream<LoginState>.empty(),
                  initialState: LoginForm());
              ...
              if (tester.firstWidget<Checkbox>(find.byType(Checkbox)).value !=
                  rememberMe) {
  ...
  testWidgets('Fields are disabled in loading state [1]', (tester) async {
    ...
    expect(
        tester
            ...
            .every((element) => element.onChanged == null),
        true);
    expect(
        tester
            .widgetList<ButtonStyleButton>(find.byType(ButtonStyleButton))
            .every((element) => element.onPressed == null),
        true);
  });
  ...
  testWidgets('On success state, page is replaced [1]', (tester) async {
    ...
    verify(() => navigatorObserver.didReplace(
        oldRoute: any(named: 'oldRoute'), newRoute: any(named: 'newRoute')));
  ...
  group('On error state, Snackbar is shown with error message [2]', () {
    ...
    for (var errorMessage in errorMessages) {
      testWidgets('On error state, Snackbar is shown with error message',
        ...
        whenListen(bloc, Stream<LoginState>.value(LoginFailure(errorMessage)),
            initialState: LoginLoading());
        await tester.pumpWidget(
          MaterialApp(
            home: BlocProvider<LoginBloc>.value(
